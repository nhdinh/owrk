...
3. **Configuration files copied** from auth-frontend-v2:
   - tailwind.config.ts
   - postcss.config.js
   - tsconfig.json
   - vite.config.ts
4. **UI Components copied** from auth-frontend-v2 (src/components/ui/)
...
Copy from auth-frontend-v2 or create:
```bash
cp services/auth-frontend-v2/src/components/ui/textarea.tsx services/asset-frontend-v2/src/components/ui/
```
...
1. **Authentication**: Shares localStorage tokens with auth-frontend-v2
