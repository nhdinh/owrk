...
class MaintenanceRepository(BaseRepository[MaintenanceRecord]):
    ...
    def get_by_asset(self, asset_id: int) -> List[MaintenanceRecord]:
        ...
        return (
            ...
            .order_by(self.model.maintenance_date.desc())
            .all()
        )
    ...
    def get_by_asset_and_status(
        self, asset_id: int, status: str
    ) -> List[MaintenanceRecord]:
        """Get maintenance records by asset and status"""
        return (
            self.session.query(self.model)
            .filter(and_(self.model.asset_id == asset_id, self.model.status == status))
            .order_by(self.model.maintenance_date.desc())
            .all()
        )
    ...
    def get_all_with_details(self) -> List[dict]:
        ...
        records = (
            self.session.query(
                MaintenanceRecord, Asset.asset_code, Asset.name.label("asset_name")
            )
        ...
        return [
            {**record[0].__dict__, "asset_code": record[1], "asset_name": record[2]}
