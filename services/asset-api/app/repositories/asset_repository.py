...
class AssetRepository(BaseRepository[Asset]):
    ...
    def get_by_code(self, asset_code: str) -> Optional[Asset]:
        ...
        return (
            ...
            .filter(Asset.asset_code == asset_code, Asset.deleted_at == None)
            .first()
        )
    ...
    def get_assets_by_status(self, status: AssetStatus) -> List[Asset]:
        ...
        return (
            self.session.query(Asset)
            .filter(Asset.status == status, Asset.deleted_at == None)
            .all()
        )
    ...
    def get_assets_by_category(self, category_id: int) -> List[Asset]:
        ...
        return (
            self.session.query(Asset)
            .filter(Asset.category_id == category_id, Asset.deleted_at == None)
            .all()
        )
    ...
    def get_assets_by_department(self, department_id: int) -> List[Asset]:
        ...
        return (
            self.session.query(Asset)
            .filter(Asset.department_id == department_id, Asset.deleted_at == None)
            .all()
        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        total = (
            self.session.query(func.count(Asset.id))
            .filter(Asset.deleted_at == None)
            .scalar()
        )

        by_status = (
            self.session.query(Asset.status, func.count(Asset.id))
            .filter(Asset.deleted_at == None)
        ...
        by_type = (
            self.session.query(Asset.asset_type, func.count(Asset.id))
            .filter(Asset.deleted_at == None)
