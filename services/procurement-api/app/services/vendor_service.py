...
class VendorService:
    ...
    def create_vendor(
        self, vendor_data: VendorCreate, created_by: int
    ) -> VendorResponse:
        """
        Create a new vendor
    ...
    def list_vendors(
        ...
        total = self.repository.count(
            status=status, search=search, min_rating=min_rating
        )
    ...
    def update_vendor(
        self, vendor_id: int, vendor_data: VendorUpdate, updated_by: int
    ) -> VendorResponse:
        """
        ...
        if vendor_data.tax_code:
            ...
            # Check uniqueness
            if not self.repository.is_tax_code_unique(
                vendor_data.tax_code, exclude_id=vendor_id
            ):
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"Tax code {vendor_data.tax_code} already exists",
    ...
    def activate_vendor(self, vendor_id: int, activated_by: int) -> VendorResponse:
        ...
        try:
            vendor = self.repository.update_status(
                vendor, VendorStatus.ACTIVE, f"Activated by user {activated_by}"
            )
            self.db.commit()
            ...
            logger.info(
                f"Vendor activated: {vendor.vendor_code} by user {activated_by}"
            )
    ...
    def deactivate_vendor(self, vendor_id: int, deactivated_by: int) -> VendorResponse:
        ...
        try:
            vendor = self.repository.update_status(
                vendor, VendorStatus.INACTIVE, f"Deactivated by user {deactivated_by}"
            )
            self.db.commit()
            ...
            logger.info(
                f"Vendor deactivated: {vendor.vendor_code} by user {deactivated_by}"
            )
    ...
    def blacklist_vendor(
        ...
        try:
            ...
            logger.warning(
                f"Vendor blacklisted: {vendor.vendor_code} by user {blacklisted_by}"
            )
    ...
    def update_rating(
        self, vendor_id: int, rating_data: VendorRatingUpdate, updated_by: int
    ) -> VendorResponse:
        ...
        try:
            ...
            logger.info(
                f"Vendor rating updated: {vendor.vendor_code} to {rating_data.rating} by user {updated_by}"
            )
