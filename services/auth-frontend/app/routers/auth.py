    ...
    if r.status_code != 200:
        return templates.TemplateResponse(
            "auth/forgot_password.html",
            {
                "request": request,
                "sent": False,
                "error": "Không thể gửi hướng dẫn đặt lại mật khẩu",
            },
            status_code=status.HTTP_400_BAD_REQUEST,
        )
    data = r.json()
...
async def get_reset_password(request: Request):
    ...
    if not token:
        return RedirectResponse(
            url="/forgot-password", status_code=status.HTTP_302_FOUND
        )
    return templates.TemplateResponse(
...
async def post_mfa_enable(request: Request, otp_code: str = Form(...)):
    ...
    async with httpx.AsyncClient() as client:
        r = await client.post(
            f"{API_BASE}/auth/mfa/enable", headers=headers, json={"otp_code": otp_code}
        )
    if r.status_code != 200:
...
async def change_password(
    ...
    async with httpx.AsyncClient() as client:
        r = await client.post(
            f"{API_BASE}/users/change-password", headers=headers, json=payload
        )
    if r.status_code != 200:
        return RedirectResponse(
            url="/profile/security?error=change", status_code=status.HTTP_302_FOUND
        )
    return RedirectResponse(
        url="/profile/security?ok=change", status_code=status.HTTP_302_FOUND
    )
...
async def mfa_disable(
    ...
    async with httpx.AsyncClient() as client:
        r = await client.post(
            f"{API_BASE}/auth/mfa/disable", headers=headers, json=payload
        )
    if r.status_code != 200:
        return RedirectResponse(
            url="/profile/security?error=mfa", status_code=status.HTTP_302_FOUND
        )
    return RedirectResponse(
