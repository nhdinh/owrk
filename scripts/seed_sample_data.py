...
def create_roles():
    ...
    for role_data in roles:
        response = requests.post(
            f"{AUTH_API}/roles", headers=get_headers(), json=role_data
        )
        ...
        if response.status_code == 201:
            ...
            print(
                f"[WARN]  Role '{role_data['display_name']}' may already exist or failed: {response.status_code}"
            )
...
def create_users():
    ...
    for user_data in users:
        response = requests.post(
            f"{AUTH_API}/users", headers=get_headers(), json=user_data
        )
        ...
        if response.status_code == 201:
            ...
            print(
                f"[OK] Created user: {user['full_name']} ({user['email']}) - ID: {user['id']}"
            )
        else:
            print(
                f"[WARN]  User '{user_data['email']}' may already exist: {response.status_code}"
            )
            ...
            if response.status_code == 200:
                ...
                existing = next(
                    (u for u in all_users if u["email"] == user_data["email"]), None
                )
                if existing:
...
def create_categories():
    ...
    for cat_data in categories:
        ...
        if response.status_code == 201:
            ...
            print(
                f"[OK] Created category: {category['name']} ({category['code']}) - ID: {category['id']}"
            )
        else:
            print(
                f"[WARN]  Category '{cat_data['code']}' may already exist: {response.status_code}"
            )
...
def create_assets(categories):
    ...
    for asset_data in assets:
        ...
        if response.status_code == 201:
            ...
            print(
                f"[OK] Created asset: {asset['name']} ({asset['asset_code']}) - ID: {asset['id']}"
            )
        else:
            print(
                f"[WARN]  Asset '{asset_data['asset_code']}' may already exist: {response.status_code}"
            )
...
def create_assignments(assets, users):
    ...
    for assign_data in assignment_data:
        # Find asset and user
        asset = next(
            (a for a in assets if a.get("asset_code") == assign_data["asset_code"]),
            None,
        )
        user = next(
            (u for u in users if u.get("email") == assign_data["user_email"]), None
        )
        ...
        assigned_date = (today - timedelta(days=assign_data["days_ago"])).strftime(
            "%Y-%m-%d"
        )
        ...
        if response.status_code == 201:
            ...
            print(
                f"[OK] Assigned {asset['asset_code']} to {user['full_name']} (ID: {assignment['id']})"
            )
        else:
            print(
                f"[WARN]  Assignment failed for {asset['asset_code']}: {response.status_code}"
            )
            print(f"   Response: {response.text}")
