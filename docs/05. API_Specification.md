...
**Authentication**: None required (internal services only)
...
- `hostname`: Required, valid hostname or IP address
- `port`: Required, integer between 1-65535
- `health_endpoint`: Required, string starting with "/"
...
Get specific service status by service ID

**Authentication**: None required

**Path Parameters:**
- `service_id`: Service name (string)
...
**Error Response:** `200 OK` (returns null if service not found)
```json
null
```
...
#### GET /reset
Reset service registry to defaults (removes all registered services except service-registry itself)

**Authentication**: None required (development only - should be disabled in production)
...
#### GET /health
Health check endpoint for service-registry itself
...
#### GET /ping
Manually trigger health checks for all registered services
...
**Notes:**
- This endpoint triggers immediate health checks for all services
- Response time is measured in milliseconds
- Status can be "healthy" or "down"
- Health check results are also logged to Redis
...
|----------|-------------|--------|---------|
| `SERVICE_HEALTH_CHECK` | Health check method | `PING`, `HTTPX` | `HTTPX` |
| `SERVICES_CACHE_FILE` | Path to service cache file | File path | `/tmp/services.json` |
| `REDIS_HOST` | Redis hostname | Hostname/IP | `localhost` |
| `REDIS_PORT` | Redis port | Port number | `6379` |
| `MONGODB_URL` | MongoDB connection URL | Connection string | - |

**Health Check Methods:**

1. **PING (ICMP)**
   - Uses raw ICMP sockets to send echo requests
   - Requires root/administrator privileges in Docker container
   - Measures network-level connectivity
   - Response time includes network latency only

2. **HTTPX (HTTP)**
   - Sends HTTP GET requests to service health endpoints
   - No special privileges required
   - Measures application-level health
   - Response time includes HTTP processing time

**Automated Polling:**
- Frequency: Every 10 minutes (configurable)
- Scheduler: APScheduler with AsyncIO
- All registered services are checked automatically
- Results are stored in Redis with timestamp keys

**Redis Logging Schema:**
```
Key: ping_logs:{timestamp}
Value: {
  "name": "service-name",
  "hostname": "hostname",
  "address": "IP",
